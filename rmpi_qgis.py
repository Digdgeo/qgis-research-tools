...
class RandomMoveGeometriesInside(QgsProcessingAlgorithm):
    ...
    def shortHelpString(self):
        return self.tr(
            ...
            'Center of gravity (CoG) and geometric centroid (CC) by geometry type:\n'
            '• Points: CoG = mean of coordinates (each point = one observation). '
            'CC = center of the bounding box. Both are meaningful and may differ.\n'
            '• Lines / Polygons: CoG = mean of geometric centroids (invariant to vertex density). '
            'When each group has a single feature, CoG and CC will always coincide.\n\n'
            'Containment modes:\n'
            '• Center of gravity only (default): the CoG lands inside the area. '
